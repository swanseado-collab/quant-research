...
# Currency policy: BTC and KOSPI sleeves park in KRW short-rate when risk-off; US sleeves/base T-bill stay USD.
def holding_ccy_local(a,active):
    if a in ('btc','k200'): return 'KRW'
    return 'USD'
v.holding_ccy=holding_ccy_local

# KR short rate: OECD/FRED monthly interbank 3m. Use a conservative 45-day information lag to avoid revised-month lookahead.
def add_kr_rate(d):
    ...
    x.obs_date=pd.to_datetime(x.obs_date); x.kr_yield=pd.to_numeric(x.kr_yield,errors='coerce'); x=x.dropna().sort_values('obs_date')
    x['available_date']=x.obs_date+pd.Timedelta(days=45)
    z=pd.merge_asof(d.sort_values('date'),x[['available_date','kr_yield']].sort_values('available_date'),left_on='date',right_on='available_date',direction='backward')
...
# Local cash transactions use KR-listed KOFR-type ETF fee assumptions.
...
# Build a no-lookahead version of sim with local/foreign cash accrual separated.
src=inspect.getsource(v.sim)
# cash factor: prior close -> current open only for USD cash
...
# Current open -> close FX applies only to USD cash, after all open transactions/rebalancing.
...
# terminal FX classification using pre-liquidation currency
...
# Only the direct-US route is relevant here. Domestic ETFs already wrap FX without external conversion.
v.ROUTES=['DIRECT_US']
v.SIZES=[100_000_000.,300_000_000.,1_000_000_000.]
