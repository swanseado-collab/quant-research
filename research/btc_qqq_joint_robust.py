...
def apply_drift(curve, alpha, annual_log_edge, dates):
...
def summarize_arrays(cagr,mdd):
    return {'cohorts':cagr.shape[0],'median_cagr':np.median(cagr,axis=0),'p10_cagr':np.quantile(cagr,.1,axis=0),'worst_cagr':np.min(cagr,axis=0),'median_mdd':np.median(mdd,axis=0),'worst_mdd':np.min(mdd,axis=0)}
...
def evaluate(cache,W,be,qe):
    ...
    for ba,qa in itertools.product(BTC_ALPHAS,QQQ_ALPHAS):
        ...
        for x in cache:
            c=x['curves'].copy()
            c[:,2]=apply_drift(c[:,2],ba,be,x['dates'])
            c[:,1]=apply_drift(c[:,1],qa,qe,x['dates'])
...
def select_robust(S,W):
    ...
    for goal in GOALS:
        mats=[]; benchmarks={}; feasible=np.ones(len(W),dtype=bool)
        ...
        for (ba,qa),z in mats:
            r=z.iloc[ix]; rr={'goal_mdd':goal,'btc_alpha':ba,'qqq_alpha':qa,'candidate_index':int(ix)}
...
def oos_selected(sel,oos_store):
    rows=[]
...
def main():
    W=stress.candidate_weights(); d,sr,qr=kt.prepare(K_RULE); err=stress.validate_fast(d,W); cache=stress.make_cache(d)
    ...
    OO=oos_selected(SEL,Ostore); OO.to_csv(OUT/'robust_oos_by_scenario.csv',index=False)
